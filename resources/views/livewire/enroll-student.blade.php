<div class="grid grid-cols-12 gap-4 md:gap-6">
    @php
        // Pass current page IDs as strings
        $pageIds = $students->pluck('id')->map(fn($i) => (string) $i)->all();
    @endphp
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
            ...
            <div class="mt-6 mb-4">
                <input
                    ...
                    class="w-full sm:w-1/3 px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring focus:ring-blue-200 focus:outline-none text-sm"
                >
            </div>
            ...
            <x-ui.bulk-container
                ...
                confirmText="This will cancel the selected student(s) enrollment."
                confirmButtonText="Yes, cancel"
                cancelButtonText="No, keep"
                successTitle="Canceled"
                successText="Enrollment canceled successfully."
            >
                <!-- Table Container -->
                <div class="w-full overflow-x-auto" wire:key="students-page-{{ $students->currentPage() }}">
                    <table class="min-w-full">
                        <!-- Table Header -->
                        <thead>
                            <tr class="border-gray-100 border-y dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30">
                                <th class="py-4 px-3 text-left">
                                    {{-- Select-all: this component must use x-init="$el._ids = @js($ids)" internally --}}
                                    <x-ui.bulk-select-all :ids="$pageIds" wire:key="sa-{{ $students->currentPage() }}" />
                                </th>
                                <th class="py-4 px-3 text-left">
                                    <p class="font-semibold text-gray-600 text-sm dark:text-gray-300 font-['Open_Sans']">#</p>
                                </th>
                                <th class="py-4 px-3 text-left">
                                    <p class="font-semibold text-gray-600 text-sm dark:text-gray-300 font-['Open_Sans']">Student Details</p>
                                </th>
                                <th class="py-4 px-3 text-left">
                                    <p class="font-semibold text-gray-600 text-sm dark:text-gray-300 font-['Open_Sans']">Contact Info</p>
                                </th>
                                <th class="py-4 px-3 text-center">
                                    <p class="font-semibold text-gray-600 text-sm dark:text-gray-300 font-['Open_Sans']">Actions</p>
                                </th>
                            </tr>
                        </thead>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            @forelse ($students as $student)
                                <tr
                                    ...
                                    x-bind:class="{ 'bg-gray-100 dark:bg-gray-700': has('{{ $student->id }}') }"
                                    class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors duration-200"
                                >
                                    <!-- Checkbox -->
                                    <td class="py-4 px-3">
                                        {{-- Row checkbox: inside the component ensure it binds to "tick" and uses wire:key="chk-{{ $value }}" --}}
                                        <x-ui.bulk-checkbox :value="$student->id" />
                                    </td>

                                    <!-- # -->
                                    <td class="py-4 px-3">
                                        <div class="flex items-center">
                                            <span class="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center text-sm font-semibold text-blue-600 dark:text-blue-400">
                                                {{ $loop->iteration }}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-3">
                                        <div class="flex items-center justify-center gap-2">
                                            <button
                                                wire:click="view_student({{ $student->id }})"
                                            ...
                                            <button
                                                wire:click="enroll_student({{ $student->id }})"
                                            ...
                                            >
                                                ...
                                                <div wire:loading wire:target="edit_student({{ $student->id }})" class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                                            </button>
                                        </div>
                                    </td>
                                </tr>
            ...
            <script>
                window.addEventListener('student-update', event => {
                    ...
                    const Toast = Swal.mixin({
                        ...
                        customClass: { popup: 'font-["Open_Sans"]' },
                        didOpen: (toast) => {
                            toast.onmouseenter = Swal.stopTimer;
                            toast.onmouseleave = Swal.resumeTimer;
                        }
                    });
                    Toast.fire({ icon: data.icon, title: data.text });
                });
            </script>
        @endpush
    </div>
